...
class DecoderSCVIGeneCell(DecoderSCVI):
    ...
    def forward(
        ...
        *cat_list: int,
        ...
        interpolation_function: torch.Tensor,
    ):
        """The forward computation for a single sample.
        ...
        # )  # torch.clamp( , max=12)
        px_rate = (
            torch.exp(library) * (cell_offset)
        ) * px_scale  # torch.clamp( , max=12)
...
def RunVAE(
    ...
    interpolation_function = interpolate.interp1d(
        sct_model_paras_withgmean["gmean"],
        sct_model_paras_withgmean["theta"],
        bounds_error=False,
    )
